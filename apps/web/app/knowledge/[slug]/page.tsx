...
export default function KnowledgeDetailPage() {
  ...
  useEffect(() => {
    if (params.slug) {
      knowledgeApi
        .getTopicContent(params.slug as string)
        .then(setContent)
        .catch(() => setError(true))
      ...
      <div className="max-w-5xl mx-auto p-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex gap-3 text-yellow-800 text-sm">
            ...
            <p>
              <strong>Not Legal Advice:</strong> This content is for educational
              purposes only. Always consult with a qualified immigration
            ...
            <ReactMarkdown
              ...
              components={{
                ...
                h1: ({ node, ...props }) => (
                  <h1
                    {...props}
                    className="text-3xl font-bold mb-6 pb-2 border-b border-gray-100"
                  />
                ),
                h2: ({ node, ...props }) => (
                ...
                ul: ({ node, ...props }) => (
                  <ul {...props} className="list-disc pl-6 space-y-1 mb-4" />
                ),
                ol: ({ node, ...props }) => (
                  <ol {...props} className="list-decimal pl-6 space-y-1 mb-4" />
                ),
              }}
            >
              {content.content}
