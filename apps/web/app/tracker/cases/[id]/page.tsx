...
export default function CaseDashboard() {
  ...
  const caseId = idParam
    ? parseInt(Array.isArray(idParam) ? idParam[0] : idParam)
    : 0;
  ...
  const handleUpdateCase = async (updatedCase: CaseEntry) => {
    try {
      ...
      if (updatedCase.priority_date === "")
        updatedCase.priority_date = undefined;
  ...
  const handleCheckStatus = async () => {
    ...
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(
        () =>
          reject(
            new Error(
              "Request timed out. Please check your internet or try again later.",
            ),
          ),
        25000,
      ),
    ...
    try {
      ...
      if (isError) {
        toast.error(
          `${status}: ${detail || "The USCIS server is unreachable."}`,
          { duration: 6000 },
        );
    ...
    <div className="min-h-screen bg-gray-50 pb-12">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-8 py-8">
          <Link
            href="/tracker/cases"
            className="flex items-center text-gray-500 hover:text-gray-900 mb-6 w-fit"
          >
      ...
      <div className="bg-blue-50 border-b border-blue-100 text-center py-2 px-4 shadow-sm relative z-10">
        <p className="text-xs text-blue-800 flex items-center justify-center gap-2">
          <AlertCircle size={14} />
          <span className="font-semibold">Not Legal Advice:</span> LifeBridge is
          a self-help tool. Consult an attorney for legal advice.
        </p>
      </div>
            ...
            <div className="relative border-l-2 border-gray-100 ml-3 space-y-8 pl-8 py-2">
              ...
              {events.map((evt, idx) => (
                <div key={idx} className="relative group">
                  <div
                    className={`absolute -left-[41px] h-6 w-6 rounded-full border-4 border-white shadow-sm flex items-center justify-center
                ...
                <div className="relative group opacity-50 grayscale">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg border border-transparent border-dashed border-gray-300">
                    <h3 className="text-lg font-semibold text-gray-500">
                      Upcoming: Biometrics Appointment
                    </h3>
                    <p className="text-gray-400 mt-1 text-sm">
                      Estimated 3-5 months after filing
                    </p>
                  </div>
                </div>
              )}
            ...
            >
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Gather tax returns"
                />
              </div>
