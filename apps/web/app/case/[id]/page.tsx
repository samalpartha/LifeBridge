...
export default function CasePage() {
  ...
  const [translatedOutputs, setTranslatedOutputs] = useState<Outputs | null>(
    null,
  );
    ...
    const translateContent = async () => {
      ...
      try {
        ...
        if (newOutputs.case.summary) {
          newOutputs.case.summary = await translateDynamic(
            newOutputs.case.summary,
          );
        }

        // Translate Checklist
        await Promise.all(
          newOutputs.checklist.map(async (item: any) => {
        ...
        // Translate Risks
        await Promise.all(
          newOutputs.risks.map(async (item: any) => {
        ...
        // Translate Timeline
        await Promise.all(
          newOutputs.timeline.map(async (item: any) => {
            item.label = await translateDynamic(item.label);
            if (item.notes) item.notes = await translateDynamic(item.notes);
          }),
        );
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button disabled={busy} onClick={onAnalyze} className="btn btn-primary">
          {busy ? (
      ...
      <div className="card bg-gradient-to-r from-indigo-50 to-purple-50 border-2 border-indigo-200">
        <div className="flex items-center mb-4">
          ...
          {isSaving && (
            <span className="ml-3 text-xs text-indigo-600 animate-pulse">
              Saving...
            </span>
          )}
        </div>
        ...
        <p className="text-sm text-gray-600 mb-4">
          Share your situation in your own words. You can type or use your
          voice. This helps us understand your context better.
        </p>
      ...
      <div className="card bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200">
        ...
        <p className="text-sm text-gray-600 mt-3">
          💡 Upload PDFs or images. OCR will extract text automatically, then AI
          ...
          <div className="mt-4 border-t border-blue-100 pt-4 animate-slide-down">
            ...
            <ul className="space-y-2">
              {documents.map((doc) => (
                <li
                  key={doc.id}
                  className="flex items-center justify-between text-sm text-gray-600 bg-white p-3 rounded-lg border border-blue-100 shadow-sm hover:shadow-md transition-shadow"
                >
      ...
      {displayOutputs && displayOutputs.case.summary && (
        <div className="card bg-gradient-to-r from-purple-50 to-pink-50 border-l-4 border-purple-500">
          <div className="flex items-start">
            <svg
              className="w-6 h-6 text-purple-600 mr-3 mt-1"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <div className="flex items-center mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {displayOutputs.checklist.length} items
                </p>
              </div>
            </div>
            <div className="space-y-3">
              {displayOutputs.checklist.map((i) => (
                ...
                >
                    ...
                    <div className="flex items-center flex-1 mr-4">
                      <label className="flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={async () => {
                            const newStatus =
                              i.status === "done" ? "todo" : "done";
                            // Optimistic update
                            if (outputs) {
                              const newOutputs = { ...outputs };
                              const item = newOutputs.checklist.find(
                                (c) => c.id === i.id,
                              );
                        ...
                        />
                        <h3
                          className={`font-semibold cursor-pointer select-none ${i.status === "done" ? "text-gray-400 line-through" : "text-gray-900"}`}
                        >
                          {i.label}
                        </h3>
                      </label>
                  ...
                  <p
                    className={`text-sm mb-2 ${i.status === "done" ? "text-gray-400" : "text-gray-600"}`}
                  >
                    {i.notes}
                  </p>
                  <Evidence
                    outputs={displayOutputs}
                    ids={i.evidence_chunk_ids}
                  />
                </div>
              ))}
            </div>
          ...
          <div className="card">
            <div className="flex items-center mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {displayOutputs.timeline.length} tasks
                </p>
              </div>
            </div>
            <div className="space-y-3">
              {displayOutputs.timeline.map((i) => (
                ...
                >
                      ...
                      <label className="flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={async () => {
                            ...
                            if (outputs) {
                              const newOutputs = { ...outputs };
                              const item = newOutputs.timeline.find(
                                (t) => t.id === i.id,
                              );
                              if (item) item.status = newStatus;
                              setOutputs(newOutputs);
                  ...
                  <p
                    className={`text-sm mb-2 ml-9 ${i.status === "done" ? "text-gray-400" : "text-gray-600"}`}
                  >
                    {i.notes}
                  </p>
                  <div className="ml-9">
          ...
          <div className="card">
            <div className="flex items-center mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {displayOutputs.risks.length} identified
                </p>
              </div>
            </div>
            <div className="space-y-3">
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex flex-col items-end ml-4">
                      <span
                        className={`badge ${getSeverityColor(i.severity)} mb-1 capitalize`}
                      >
                        {i.severity} Risk
                      </span>
        ...
        <div className="card text-center py-12">
          ...
          <div className="space-y-2">
            ...
            <p className="text-gray-500 text-sm">
              Extracting evidence, identifying risks, and building your
              timeline.
            </p>
          </div>
        </div>
