  ...
  const fetchAttorneys = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/attorneys/search?${params.toString()}`,
      );
      ...
      const mappedAttorneys: Attorney[] = results.map((record: any) => ({
        ...
        image:
          record.image ||
          `https://api.dicebear.com/7.x/initials/svg?seed=${record.name}`,
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="text-sm text-gray-500">{t("attorneys.verified")}</div>
        </div>
      </div>
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
            ...
            <div className="space-y-3">
              ...
              >
                {loading
                  ? t("attorneys.searching")
                  : t("attorneys.searchButton")}
              </button>
            </div>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <span className="text-xs text-gray-400">
                        {attorney.reviews} {t("attorneys.reviews")}
                      </span>
                      {attorney.confidence_score && (
                        <span className="text-[10px] bg-green-50 text-green-600 px-1.5 py-0.5 rounded border border-green-100">
                          {(attorney.confidence_score * 100).toFixed(0)}%{" "}
                          {t("attorneys.match")}
                        </span>
                      )}
                    </div>
