...
function providerLabelFor(source: RetrievalSource): string {
    if (source.provider === "digitalocean_gradient" || source.type?.startsWith("gradient_")) {
        return "DigitalOcean Gradient";
    }
    if (source.provider?.startsWith("openstreetmap") || source.type?.startsWith("openstreetmap_")) {
        return "OpenStreetMap";
    }
    if (source.provider === "lifebridge_operational_db" || source.type === "lifebridge_havens_snapshot") {
        return "Live Ops DB";
    }
...
function prioritizeSources(sources: RetrievalSource[]): RetrievalSource[] {
    const score = (source: RetrievalSource) => {
        ...
        if (source.provider === "digitalocean_gradient" || source.type?.startsWith("gradient_")) total += 40;
        if (typeof source.confidence === "number") total += Math.round(source.confidence * 10);
        return total;
    };
...
export default function SamaritanChat() {
    ...
    const [runtimeHint, setRuntimeHint] = useState("Checking DigitalOcean live retrieval...");
        ...
        const checkRuntime = async () => {
            try {
                ...
                if (response.ok && data.active_mode === "live") {
                    ...
                    setRuntimeHint("Live retrieval is unavailable. Check Gradient credentials.");
                }
            } catch {
                if (!isActive) return;
                setRuntimeState("offline");
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (locationContext) {
                ...
                chatContext.location_accuracy_m = Math.round(locationContext.accuracyMeters || 0);
                ...
                try {
                    ...
                    if (havensRes.ok) {
                        ...
                        if (Array.isArray(havenData)) {
                            chatContext.nearby_havens_snapshot = havenData.slice(0, 4).map((haven) => ({
                                name: haven.name,
                                type: haven.type,
            ...
            const responseText = data.response?.trim() || "I could not generate a response. Please try again.";
            ...
            const errorText = e instanceof Error ? e.message : "Connection error. Please try again.";
            setMessages((prev) => [
                ...prev,
                {
                    id: Date.now().toString(),
    ...
    const sendDisabled = !inputValue.trim() || isTyping || runtimeState === "offline";
                ...
                {isOpen && (
                    ...
                    >
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center space-x-1">
                                        ...
                                        <span className="text-blue-100 text-xs">DigitalOcean Retrieval Assistant</span>
                                    </div>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="px-4 py-2 border-b border-slate-200 bg-white">
                            <div className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-medium ${runtimeBadgeClass}`}>
                                {runtimeState === "live" ? (
                        ...
                        <div className="h-80 overflow-y-auto p-4 space-y-4 bg-slate-50/80">
                            {messages.map((msg) => {
                                const sortedSources = prioritizeSources(Array.isArray(msg.sources) ? msg.sources : []);
                                const displaySources = sortedSources.slice(0, 5);
                                const realtimeCount = sortedSources.filter(isRealtimeSource).length;
                                return (
                                    <div
                                        key={msg.id}
                                        ...
                                        >
                                            ...
                                            {msg.role === "assistant" && displaySources.length > 0 && (
                                                <div className="mt-2 space-y-1">
                                                    <p className="text-[10px] uppercase tracking-wide text-gray-400">
                                                        Sources{realtimeCount > 0 ? ` · ${realtimeCount} live` : ""}
                                                    </p>
                                            ...
                                            {msg.role === "assistant" && msg.traceId && (
                                                <p className="mt-2 text-[10px] text-gray-400">
                                                    Trace: <span className="font-mono">{msg.traceId.slice(0, 8)}</span>
                                                </p>
                                            )}
                        ...
                        <div className="p-4 bg-white border-t border-gray-100">
                            <div className="flex items-center space-x-2">
                                <input
                                    ...
                                    placeholder={runtimeState === "offline" ? "Live retrieval unavailable" : "Ask Samaritan..."}
