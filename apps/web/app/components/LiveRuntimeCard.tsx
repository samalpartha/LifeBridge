    ...
    return (
        <div className="surface-card shimmer-border animate-enter border border-indigo-100 bg-white/90 p-5 backdrop-blur">
            <div className="flex items-center justify-between">
                ...
                <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-semibold ${live ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}>
                    <span className={live ? "live-dot" : "h-2 w-2 rounded-full bg-amber-500"} />
                    {runtime?.active_mode?.toUpperCase() || "UNKNOWN"}
                </span>
            </div>
            <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-wide text-slate-500">Configured</p>
                    <p className="mt-1 text-sm font-semibold text-slate-900">{runtime?.configured_mode?.toUpperCase() || "N/A"}</p>
                </div>
                <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-wide text-slate-500">Last Sync</p>
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-wide text-slate-500">Clock</p>
                    <p className="mt-1 flex items-center gap-1 text-sm font-semibold text-slate-900">
