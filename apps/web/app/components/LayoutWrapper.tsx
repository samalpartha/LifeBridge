...
function pageMeta(pathname: string | null) {
    if (!pathname || pathname === "/") {
        return {
            ...
            subtitle: "Choose the right workflow: crisis response, case continuity, or guidance.",
        };
    }
    if (pathname.startsWith("/tracker")) {
        return {
            title: "Case Workspace",
            subtitle: "Track timeline evidence, tasks, and filing readiness in one place.",
        };
    }
    if (pathname.startsWith("/knowledge")) {
        return {
            title: "Knowledge Base",
            subtitle: "Find practical legal and migration guidance with searchable resources.",
        };
    }
    ...
    if (pathname.startsWith("/vault")) {
        return {
            ...
            subtitle: "Store and retrieve critical identity and case documents securely.",
        };
    }
    if (pathname.startsWith("/help")) {
        return {
            title: "Help Center",
            subtitle: "Get actionable support guidance and emergency resources quickly.",
        };
    }
    return {
        title: "LifeBridge Workspace",
        subtitle: "Complete actions with clear context and source-backed assistance.",
    };
}
...
export function LayoutWrapper({ children }: { children: React.ReactNode }) {
    ...
    const isCrisisExperiencePage =
        pathname === "/crisis" || pathname?.startsWith("/crisis/") || pathname?.startsWith("/reunion");
    ...
    return (
        <div className="flex min-h-screen bg-slate-100">
            ...
            <main className={`flex-1 min-h-screen flex flex-col relative ${showSidebar ? "ml-64" : "w-full"}`}>
                {showSidebar && (
                    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur">
                        <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-4">
                            <div className="animate-enter">
                                ...
                                <h2 className="mt-1 text-xl font-semibold text-slate-900">{meta.title}</h2>
                            ...
                            <div className="animate-enter stagger-1 flex flex-wrap items-center justify-end gap-2">
                                <div className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${runtime?.active_mode === "live" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}>
                                    <span className={runtime?.active_mode === "live" ? "live-dot" : "h-2 w-2 rounded-full bg-amber-500"} />
                ...
                <div className="flex-1 overflow-y-auto">
                    {showSidebar ? (
                        <div className="mx-auto w-full max-w-7xl p-6 lg:p-8 animate-enter-slow">{children}</div>
                    ) : (
                        children
                    )}
                </div>
