...
class LLMProvider(ABC):
    @abstractmethod
    def generate_case_plan(self, scenario: str, user_story: str, chunks: list[str]) -> dict[str, Any] | None:
        pass
...
class GeminiProvider(LLMProvider):
    ...
    def generate_case_plan(self, scenario: str, user_story: str, chunks: list[str]) -> dict[str, Any] | None:
        try:
            ...
            prompt = (
                "You are an expert immigration legal assistant. Your task is to analyze the user's "
                "situation and documents to generate a precise, actionable case plan.\n\n"
                "Context:\n"
                ...
                "{\n"
                '    "checklist": [\n'
                '        { "label": "Action Item Title", "notes": "Detailed instructions", '
                '"status": "todo", "evidence_keywords": ["keyword1", "keyword2"] }\n'
                "    ],\n"
                '    "timeline": [\n'
                '        { "label": "Task Title", "due_date": "Date or Timeframe", "owner": "user", '
                '"notes": "Why this matters", "evidence_keywords": ["keyword"] }\n'
                "    ],\n"
                '    "risks": [\n'
                '        { "category": "category_name", "severity": "high/medium/low", '
                '"statement": "Risk Title", "reason": "Explanation why", '
                '"evidence_keywords": ["keyword"] }\n'
                "    ]\n"
                "}\n\n"
                "Instructions:\n"
                '1. Analyze the User Story heavily. If they say "I don\'t have a stamp", that is a '
                "critical fact.\n"
                "2. Cross-reference with Document Extracts. If a document is missing (e.g., Marriage "
                "Cert mentioned but not found), flag it as a risk.\n"
                "3. Be specific. Do not give generic advice. Give advice tailored to the visible facts.\n"
                '4. For "evidence_keywords", return 2-3 unique words from the source text that justify '
                "your finding."
            )
...
class LocalAIProvider(LLMProvider):
    ...
    def _call_completion(self, messages: list[dict[str, str]], json_mode: bool = False) -> str | None:
...
def generate_chat_response(message: str, history: list[dict[str, str]] | None = None) -> str:
    provider = get_llm_provider()
    return provider.chat(message, history or [])
